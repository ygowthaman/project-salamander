...
export const categories = pgTable(
  ...
  (t) => [
    // Case-insensitive per user: a second "groceries" is a 409, not a row.
    // Unlike users.email this is a lower(name) *expression* index rather than a
    // normalise-on-write convention, because the display casing the user typed
    // is worth keeping — "Books", not "books".
    uniqueIndex("categories_user_id_name_unique").on(t.userId, sql`lower(${t.name})`),
  ],
);
