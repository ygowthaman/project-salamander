...
**`PRD.md` is authoritative.** Where code disagrees with it, the code is wrong.
...
- [ ] **The remaining item routes return 501** via `notImplemented` in
      [`api/inventory.ts`](../../node-server/src/api/inventory.ts) — the flat list and stock. The
      `inventoryItems` repository is written and nothing calls those parts of it, so this is wiring
      rather than design.
...
- [ ] **No inventory tests.** [`node-server/test/`](../../node-server/test/) holds only
      ...
      reorder-related.
- [ ] **No attribution on the card.** The wire shape carries `added_by` and `is_private`; nothing
      renders either.
