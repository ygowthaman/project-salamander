...
- **Quantity** — how much is on hand. Mandatory, and at least one when the item is created: *"add milk"* means one carton, not an unspecified amount, so there is nothing for the user to leave blank and nothing for the interpreter to guess. It may fall to zero afterwards — having run out is the state the inventory most needs to show, and it is why the count is a number on the row rather than a flag.
